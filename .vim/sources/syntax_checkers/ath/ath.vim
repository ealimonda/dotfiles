...
" checking execution via the variable 'g:syntastic_ath_compiler_options':
"
"   let g:syntastic_ath_compiler_options = ' -my_custom_options'
"
" Additionally the setting 'g:syntastic_ath_config_file' allows you to define a
" file that contains additional compiler arguments.
" The file is expected to contain one option per line. If none is
" given the filename defaults to '.syntastic_ath_config':
"
"   let g:syntastic_ath_config_file = '.config'
"
" Use the variable 'g:syntastic_ath_errorformat' to override the default error
" format:
"
"   let g:syntastic_ath_errorformat = '%f:%l:%c: %trror: %m'
"
" Set your compiler executable with e.g. (defaults to script-checker)
"
"   let g:syntastic_ath_compiler = '/path/to/Hercules/script-checker'

if exists('g:loaded_syntastic_ath_hercules_checker')
    finish
endif
let g:loaded_syntastic_ath_hercules_checker = 1

if !exists('g:syntastic_ath_compiler')
    let g:syntastic_ath_compiler = 'script-checker'
endif

function! SyntaxCheckers_ath_hercules_IsAvailable()
    return executable(g:syntastic_ath_compiler)
endfunction

let s:save_cpo = &cpo
set cpo&vim

if !exists('g:syntastic_ath_compiler_options')
    let g:syntastic_ath_compiler_options = ''
endif

if !exists('g:syntastic_ath_config_file')
    let g:syntastic_ath_config_file = '.syntastic_ath_config'
endif

function! SyntaxCheckers_ath_hercules_GetLocList() dict
    ...
    "if exists('g:syntastic_ath_errorformat')
    "    let errorformat = g:syntastic_ath_errorformat
    "endif

    " add optional user-defined compiler options
    "let makeprg .= g:syntastic_ath_compiler_options

    "let makeprg .= ' ' . syntastic#util#shexpand('%')

    " add optional config file parameters
    "let makeprg .= ' ' . syntastic#c#ReadConfig(g:syntastic_ath_config_file)
