...
sub print_anames
{
  ...
  if (!$channel) {
    ...
    #                MSGLEVEL_CLIENTCRAP);
    my $m = Irssi::current_theme()->get_format('fe-common/core', 'names');
    $m =~ s/\$0/\Q${chan}/;
    $m = Irssi::current_theme()->format_expand($m);
    $channel->print($m, MSGLEVEL_CLIENTCRAP);
    columnize_nicks($channel,@nicks);
    $m = Irssi::current_theme()->get_format('fe-common/core', 'endofnames');
    $m =~ s/\$0/\Q${chan}/; $m =~ s/\$1/\Q${total}/; $m =~ s/\$2/\Q${ops}/;
    $m =~ s/\$3/\Q${halfops}/; $m =~ s/\$4/\Q${voices}/; $m =~ s/\$5/\Q${normal}/;
    $m =~ s/normal/normal {hilight \Q${away}} away/;
    $m = Irssi::current_theme()->format_expand($m);
    $channel->print($m, MSGLEVEL_CLIENTNOTICE);
...
sub columnize_nicks
{
  ...
  for (my $i = 0; $i < $cols; $i++) {
    ...
    for (my $j = 0; $j < $rows; $j++) {
      ...
      if ($n ne "") {
        $r[$j] .= "%K<%n$n" . fill_spaces($n,$max_length) . "%K>%n "; # FIXME: Use theme formats
      }
    }
  }
