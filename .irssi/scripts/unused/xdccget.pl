...
sub event_message_irc_notice {
	...
	} elsif (/\** All Slots Full, Added (|you to the main )queue in position ([0-9]*)/i || /\** You can only have [0-9]* transfer(|s) at a time, Added you to the main queue for pack .* in position ([0-9]*)\./i || /\** All Slots Full, Added you to the main queue for pack (.*) in position ([0-9]*)/i ) {
	    $queue[0]->{'pos'} = $2;
	    $queue[0]->{'etr'} = 0;
	    $queue[0]->{'status'} = 2;
