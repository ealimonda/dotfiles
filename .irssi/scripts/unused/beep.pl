...
sub beep {
	...
	if( $server && ! Irssi::settings_get_bool($server->{usermode_away} ? "beep_when_away" : "beep_when_not_away")) {
		Irssi::signal_stop();
	} else {
...
Irssi::settings_add_bool("lookandfeel", "beep_when_not_away", 1);
#Irssi::settings_add_str("misc", "beep_command",
#			"esdplay ~/sound/events/beep.wav &");
Irssi::settings_add_str("misc", "beep_command", "beep -l 125 -f 392 -D 50 --new -l 125 -f 392 -D 50 &");
