...
Irssi::settings_add_str(setc, set 'display_nokey', ' $N%K:%n$H$C$S');
Irssi::settings_add_str(setc, set 'display_key', ' $Q%K/%n$N%K:%n$H$C$S');
Irssi::settings_add_str(setc, set 'display_nokey_active', '%r>%n $N%K:%n$H$C$S');
Irssi::settings_add_str(setc, set 'display_key_active', '%r>%n $Q%K/%n$N%K:%n$H$C$S');
Irssi::settings_add_str(setc, set 'display_header', '');
Irssi::settings_add_str(setc, set 'separator', "|");
Irssi::settings_add_bool(setc, set 'prefer_name', 1);
...
Irssi::settings_add_str(setc, set 'placement', 'top');
...
Irssi::settings_add_str(setc, set 'mouse_click', "/mouse_tempdisable");
Irssi::settings_add_str(setc, set 'mouse_middle_click', "/window goto active");
Irssi::settings_add_str(setc, set 'mouse_right_click', "/window last");
Irssi::settings_add_str(setc, set 'mouse_gesture_up', "/scrollback home");
Irssi::settings_add_str(setc, set 'mouse_gesture_down', "/scrollback end");
Irssi::settings_add_str(setc, set 'mouse_gesture_left', "/window prev");
Irssi::settings_add_str(setc, set 'mouse_gesture_right', "/window next");
Irssi::settings_add_str(setc, set 'mouse_right_gesture_up', "");
Irssi::settings_add_str(setc, set 'mouse_right_gesture_down', "");
Irssi::settings_add_str(setc, set 'mouse_right_gesture_left', "");
Irssi::settings_add_str(setc, set 'mouse_right_gesture_right', "");
Irssi::settings_add_str(setc, set 'mouse_middle_gesture_up', "");
Irssi::settings_add_str(setc, set 'mouse_middle_gesture_down', "");
Irssi::settings_add_str(setc, set 'mouse_middle_gesture_left', "");
Irssi::settings_add_str(setc, set 'mouse_middle_gesture_right', "");
Irssi::settings_add_str(setc, set 'mouse_scroll_up', "/scrollback goto -1");
Irssi::settings_add_str(setc, set 'mouse_scroll_down', "/scrollback goto +1");
...
sub wlreset {
	...
	if ($MOUSE_ON and $ENV{"TERM"} =~ /^rxvt|screen|xterm(-color)?$/) {
		Irssi::command_bind('mouse_xterm' => 'mouse_xterm');
		Irssi::command_bind('mouse_tempdisable' => 'mouse_tempdisable');
...
#~		}
		...
		if ($MOUSE_ON) {
			...
			Irssi::command_unbind('mouse_tempdisable' => 'mouse_tempdisable');
		}
	}
}
...
sub mouse_cmd {
	my ($setting) = @_;
	my $cmd = Irssi::settings_get_str($setting);
	return if $cmd eq '';
	Irssi::signal_emit("send command", $cmd, Irssi::active_win->{'active_server'}, Irssi::active_win->{'active'});
}
...
sub mouse_event {
	...
	if ($b == 64 or $b == 65) {
		# Scroll wheel
		mouse_cmd(set 'mouse_scroll_' . ($b == 64 ? 'up' : 'down'));
		return;
	}
	# proceed only if a button is being released
	return if $b != 3;
	return unless (0 <= $oldb && $oldb <= 2);
	# Window selection (click on the windowlist lines)
	if ($y < $currentLines and $oldy < $currentLines and $oldb == 0) {
		$x -= sb_length('{sb x}') / 2;
		return unless $x > -1;
		my $win = -1;
		my $blocklen = Irssi::settings_get_int(set 'block');
		if (!$SCREEN_MODE and $blocklen < 0) {
			$blocklen = abs($blocklen) + sb_length(Irssi::settings_get_str(set 'separator'));
			$win = int($x / $blocklen);
			#Irssi::print("window calc#1 $win ($x)");
			$win += 1;
			$win += $y * int(([Irssi::windows]->[0]{'width'} - sb_length('{sb x}')) / $blocklen);
			#Irssi::print('conditions met! changing to window ' . $win);
		}
		Irssi::command('window ' . $win) if $win >= 0;
		return;
	}
	my $button = $BUTTONS[$oldb];
	my ($dx, $dy) = ($x-$oldx, $oldy-$y, 0);
	my $distance = sqrt($dx*$dx + $dy*$dy);
	# if it was a mouse click (press and release in the same position)
	if ($distance < 3) {
		mouse_cmd(set 'mouse' . $button . "_click");
		return;
	}
	# Otherwise, find gestures
	my $angle = asin($dy/$distance) * 180 / 3.14159265358979;
	if ($angle < 20 && $angle > -20 && $dx > 0) {
		mouse_cmd(set 'mouse' . $button . "_gesture_right");
	} elsif ($angle < 20 && $angle > -20 && $dx < 0) {
		mouse_cmd(set 'mouse' . $button . "_gesture_left");
	} elsif ($angle > 40) {
		mouse_cmd(set 'mouse' . $button . "_gesture_up");
	} elsif ($angle < -40) {
		mouse_cmd(set 'mouse' . $button . "_gesture_down");
	}
}
